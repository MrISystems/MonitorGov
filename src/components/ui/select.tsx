...
export const Select = memo(
  forwardRef<HTMLSelectElement, SelectProps>(
    ({ value, onValueChange, children, label, error, helperText, className, ...props }, ref) => {
      ...
      return (
        <div className="space-y-2">
          ...
          {helperText && !error && (
            <p id={`${id}-helper`} className="text-sm text-neutral-500 dark:text-neutral-400">
              {helperText}
            </p>
          )}
        </div>
...
export const SelectTrigger = memo(({ children, className, ...props }: SelectTriggerProps) => (
  <div
    className={cn(
...
export const SelectValue = memo(({ placeholder, className }: SelectValueProps) => (
  <option value="" className={cn('text-neutral-500 dark:text-neutral-400', className)}>
    {placeholder}
  </option>
));
...
export const SelectItem = memo(({ value, children, disabled, className }: SelectItemProps) => (
  <option
    value={value}
    disabled={disabled}
    className={cn(
